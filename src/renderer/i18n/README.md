...
本项目使用 `react-i18next` 实现国际化支持，支持英文、中文、俄文切换。
...
function LanguageSwitcher() {
  ...
  const changeLanguage = (lang: 'en' | 'zh' | 'ru') => {
...
3. 在 `src/renderer/i18n/locales/ru.json` 添加俄文翻译
4. 使用 `t('key.path')` 在代码中引用
...
2. **保持层级一致** - 英文、中文和俄文的 JSON 结构要完全一致
