...
export function useApiConfigState(options: UseApiConfigStateOptions = {}) {
  ...
  const isLlm7ModelEndpoint =
    provider === 'custom' &&
    customProtocol === 'openai' &&
    normalizeBaseUrlForCompare(currentProfile.baseUrl) === LLM7_API_BASE_URL;
  const supportsLiveModelRefresh = provider === 'ollama' || isLlm7ModelEndpoint;
  ...
  const modelOptions = supportsLiveModelRefresh
  ...
  const refreshModelOptions = useCallback(async () => {
    if (!isElectron || !supportsLiveModelRefresh) {
      return [];
    }
  ...
  // Auto-refresh model list when a live model endpoint changes (debounced).
  // Only fires for URLs that look plausible (start with http(s):// and have a host).
  useEffect(() => {
    if (!supportsLiveModelRefresh) return;
  ...
  }, [supportsLiveModelRefresh, baseUrl, refreshModelOptions]);
